...
# export ONLY schema
docker exec -it orientdb console.sh "use remote:localhost/DataFusion root $ORIENTDB_ROOT_PASSWORD; export database /openedrserver/backend/exportScripts/schema.gz -includeRecords=false;"
...
docker exec -it openedr-app /bin/bash -c "cd /openedrserver/backend/exportScripts/ && node exportFunctions_ORole.js"
...
docker exec -it openedr-app /bin/bash -c "cd /openedrserver/backend/exportScripts/ && node exportFunctions.js"
